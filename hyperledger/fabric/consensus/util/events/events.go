...
type Receiver interface {
	...
	//向reciver发送一个事件，如果返回不是nil，就返回下一个事件
...
//封装了一个events channel；封装了Receiver接口；并实现了Manager接口；
type managerImpl struct {
	threaded
	receiver Receiver  //Receiver接口，封装了ProcessEvent接口，在obcBatch中通过调用SetReceiver将obcBatch设置为了这个receiver；
	events   chan Event //事件通道，接收所有的事件
}
...
func SendEvent(receiver Receiver, event Event) {
	next := event
	for {
		// 这里很关键，这层for循环使得ProcessEvent()返回的事件，再次调用ProcessEvent()。
...
//TImer是跟时间相关的事件，thread可以调用这些方法，然后发送相应的事件到event queue中
type Timer interface {
	//软重启定时器
	SoftReset(duration time.Duration, event Event) // start a new countdown（倒计时）, only if one is not already started
	//强制重新设置定时器
